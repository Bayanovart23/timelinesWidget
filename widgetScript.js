function filterPhoneNumber(n){return n.split(" ").join("").split("+").join("").split("-").join("")}function validatePhoneLink(n){return n=filterPhoneNumber(n),n.startsWith("+")||(n="+"+n),n}function getWhatsappLink(n,t,i){return"https://wa.me/"+validatePhoneLink(n)+(t!==""?"/?text="+prefieldText(i):"")}function renderWidget(n){let t=document.createElement("div"),i=[];for(let t=0;t<n.contacts.length;t++)i.push(getWhatsappLink(n.contacts[t].phone,n.text,n));let r=n.type==="ww-extended"?getExtendedHtmlString(i,n):getStandardHtmlString(i[0],n.call);return t.id="whatsapp-widget-container",t.innerHTML=r.trim(),document.body.appendChild(t),t}function prefieldText(n){if(n.toggle){const t=urlConstructor();return n.text===""?"":n.text+" "+t}return n.text}function urlConstructor(){const n=getGaCookies(),t=window.location.href.substring(window.location.href.length-1)==="/"?window.location.href+"?":window.location.href+"/?",i=n.wwGclid?`gclid=${n.wwGclid}`:"",r=n.wwGaClientId?`&gid=${n.wwGaClientId}`:"",u=n.wwGaSessionId?`&sid=${n.wwGaSessionId}`:"",f=n.wwUnixTimestampMicSec?`&uts=${n.wwUnixTimestampMicSec}`:"";return t+f+i+r+u}function insertStyles(){let t=document.getElementsByTagName("head")[0],n=document.createElement("link");n.type="text/css";n.rel="stylesheet";n.href="https://cdn.jsdelivr.net/gh/Bayanovart23/cssForScriptMain@main/styles.css";t.appendChild(n)}function getGaCookies(){function t(n){const t=document.cookie.split(";");for(const t of t){const[i,r]=t.trim().split("=");if(n.test(i))return r}return undefined}const n=t(/_ga$/),r=n?n.split(".")[2]+"."+n.split(".")[3]:undefined,i=t(/_ga_[A-Z0-9]{10}$/),u=i?i.split(".")[2]:undefined,f=(new Date).getTime()*1e3,e=new URLSearchParams(window.location.search),o=e.get("gclid");return{wwGclid:o,wwGaClientId:r,wwGaSessionId:u,wwUnixTimestampMicSec:f}}function tmWidgetInit(n){let t=getGaCookies();const i={method:"POST",headers:{"Content-type":"application/json"},body:JSON.stringify(t)};t&&fetch("https://hook.eu1.make.com/wfyknbxd9gxi7iav1prw9pprzzsrv1h4",i).then(n=>console.log(n)).catch(n=>console.log(n));let r=renderWidget(n);initAmplitude(n);insertStyles()}function readCookie(n){let t=n+"=",i=document.cookie.split(";");for(let n=0;n<i.length;n++){let r=i[n];while(r.charAt(0)==" ")r=r.substring(1,r.length);if(r.indexOf(t)==0)return r.substring(t.length,r.length)}return null}function setCookie(n,t,i){let r=new Date;r.setTime(r.getTime()+i*864e5);let u="expires="+r.toUTCString();document.cookie=n+"="+t+";"+u+"; path=/; domain=."+window.location.hostname}function initAmplitude(n){let i=function(n){readCookie("widget_attributed")===null&&t!=null&&(n.logEvent("widget_attributed"),setCookie("widget_attributed","1",365))},r={widgetType:n.type,widgetSource:"cdn",widgetDomain:window.location.hostname,clientEmail:n.email,phone:filterPhoneNumber(n.contacts[0].phone),email:n.email},t=filterPhoneNumber(n.contacts[0].phone);console.log("Amplitude Attribution: about to init the client instance");amplitude.getInstance().init("0e73dc7c6a30ebbc4bf3ea1144ebdb71",t,{includeReferrer:!0,includeUtm:!0,includeGclid:!0,saveParamsReferrerOncePerSession:!0,logAttributionCapturedEvent:!0},i);amplitude.getInstance().setUserProperties(r)}function getStandardHtmlString(n,t){return`<div id='whatsapp-widget' class='ww-standard'>
    ...
    </div>`}function toggleWidget(){document.getElementById("whatsapp-widget-extended").classList.toggle("ww-open");document.getElementById("ww-extended-icon").classList.toggle("ww-icon-open")}function getExtendedHtmlString(n,t){return`<div id='whatsapp-widget-extended' class='ww-extended'>
                <div class='ww-extended-header'>
                    ...
                    <div>
                        <h3 class="ww-title">${t.brand?t.brand:"Start a conversation"}</h3>
                        <p>${t.subtitle?t.subtitle:"Hi! Click on of our members below to chat on WhatsApp ;)"}</p>
                    </div>
                </div>

                <div class='ww-extended-body'>
                    ${t.contacts.map((t,i)=>`<a target='_blank' href="${n[i]}">
                        <div class='ww-extended-contact'>
                            <div class='ww-extended-contact-left'>
                                ${t.image?`<img src="${t.image}" alt={contact.name} />`:""}
                                <div class='ww-extended-contact-text'>
                                    <p class='ww-extended-contact-name'>
                                        ${t.name?t.name:t.title}
                                    </p>
                                    ${t.name&&`<p class='ww-extended-contact-title'>
                                        ${t.title}
                                    </p>`}
                                </div>    
                            </div>
                    ...
                    <div class='ww-extended-logo'>
                        <a target='_blank' href="https://timelines.ai/" rel="nofollow">
            ...
            </div>`}(function(n,t){function f(n,t){n.prototype[t]=function(){return this._q.push([t].concat(Array.prototype.slice.call(arguments,0))),this}}function l(n){function t(t){n[t]=function(){n._q.push([t].concat(Array.prototype.slice.call(arguments,0)))}}for(let n=0;n<c.length;n++)t(c[n])}let i=n.amplitude||{_q:[],_iq:{}},r=t.createElement("script");r.type="text/javascript";r.integrity="sha384-tzcaaCH5+KXD4sGaDozev6oElQhsVfbJvdi3//c2YvbY02LrNlbpGdt3Wq4rWonS";r.crossOrigin="anonymous";r.async=!0;r.src="https://cdn.amplitude.com/libs/amplitude-8.5.0-min.gz.js";r.onload=function(){n.amplitude.runQueuedFunctions||console.log("[Amplitude] Error: could not load SDK")};let u=t.getElementsByTagName("script")[0];u.parentNode.insertBefore(r,u);let e=function(){return this._q=[],this},o=["add","append","clearAll","prepend","set","setOnce","unset","preInsert","postInsert","remove"];for(let n=0;n<o.length;n++)f(e,o[n]);i.Identify=e;let s=function(){return this._q=[],this},h=["setProductId","setQuantity","setPrice","setRevenueType","setEventProperties"];for(let n=0;n<h.length;n++)f(s,h[n]);i.Revenue=s;let c=["init","logEvent","logRevenue","setUserId","setUserProperties","setOptOut","setVersionName","setDomain","setDeviceId","enableTracking","setGlobalUserProperties","identify","clearUserProperties","setGroup","logRevenueV2","regenerateDeviceId","groupIdentify","onInit","logEventWithTimestamp","logEventWithGroups","setSessionId","resetSessionId"];l(i);i.getInstance=function(n){return n=(!n||n.length===0?"$default_instance":n).toLowerCase(),Object.prototype.hasOwnProperty.call(i._iq,n)||(i._iq[n]={_q:[]},l(i._iq[n])),i._iq[n]};n.amplitude=i;tmWidgetInit()})(window,document);